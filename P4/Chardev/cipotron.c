...
int init_module(void)
{
    ...
    if ((cipotron=cdev_alloc())==NULL) {
        printk(KERN_INFO "cdev_alloc() failed ");
        unregister_chrdev_region(start, 1); ////Devolver rango de mayor y minor
        return -ENOMEM;
    }

    cdev_init(cipotron,&fops); //conecta el driver con la interfaz de operaciones

    if ((ret=cdev_add(cipotron,start,1))) {	//añadir "1" dispositivo chardev
        printk(KERN_INFO "cdev_add() failed ");
        kobject_put(&cipotron->kobj);
...
void cleanup_module(void)
{
    /* Destroy chardev */
    if (cipotron)
        cdev_del(cipotron); //eliminamos del cdev el dispositivod
...
{
    printk(KERN_ALERT "Sorry, this operation TE LA HACES TU MAJO.\n");
    return -EPERM;
}
