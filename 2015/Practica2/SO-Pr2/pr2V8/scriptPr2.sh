...
rm punto-montaje/fichero1.txt 
./audita disco-virtual

echo "Borramos el fichero2.txt y auditamos"
read p

rm punto-montaje/fichero2.txt 
./audita disco-virtual
...
truncate -s 16384 punto-montaje/fuseLib.c
...
diff src/fuseLib.c punto-montaje/fuseLib.c > d1.txt
if [ $? -eq 0 ] ; then
	echo "IGUALES"
else	
	echo "NO IGUALES"
...
diff src/myFS.h punto-montaje/myFS.h > h1.txt
if [ $? -eq 0 ] ; then
	echo "IGUALES"
else	
	echo "NO IGUALES"
