...
*
*/
static int my_unlink(const char *ruta){

	int idxNodoI, nodoI;
	fprintf(stderr, "--->>>my_unlikn: path %s", ruta);
	...
	int tamBloque = miSistemaDeFicheros.nodosI[nodoI]->numBloques;
	int i, indiceBloque;
	for (i = 0; i < tamBloque; ++i) {
		indiceBloque = miSistemaDeFicheros.nodosI[nodoI]->idxBloques[i];
		miSistemaDeFicheros.mapaDeBits[indiceBloque] = 0;
	}

	miSistemaDeFicheros.nodosI[nodoI]->libre = true;
	miSistemaDeFicheros.nodosI[nodoI]->numBloques = 0;
	miSistemaDeFicheros.nodosI[nodoI]->tamArchivo = 0;
	...
	escribeDirectorio(&miSistemaDeFicheros);
	escribeMapaDeBits(&miSistemaDeFicheros);
	escribeNodoI(&miSistemaDeFicheros, nodoI, miSistemaDeFicheros.nodosI[nodoI]);
 ...
 *
 */
static int my_read(const char *ruta, char *buff, size_t size, off_t offset, struct fuse_file_info *fi){

	fprintf(stderr, "--->>>my_read: Vamos a leer el siguiente archivo--> %s", path;

	int idxNodoI, nodoI;
	fprintf(stderr, "--->>>my_read: Vamos a leer el archivo: %s", ruta);
	if( (idxNodoI = buscaPosDirectorio(&miSistemaDeFicheros, (char*)ruta+1)) == -1){
		return -ENOENT;
	}
	nodoI = miSistemaDeFicheros.directorio.archivos[idxNodoI].idxNodoI;
